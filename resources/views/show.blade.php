...
<x-pertuk::pertuk-layout :title="$title">
    <x-slot:sidebar>
        @include('pertuk::components.sidebar', ['items' => \Xoshbin\Pertuk\Services\DocumentationService::make()->list(), 'active' => $slug ?? null])
    </x-slot:sidebar>
                ...
                @if($isRtl)
                    @foreach(array_reverse($breadcrumbs) as $crumb)
                        <li class="flex items-center">
                            @if($crumb['slug'])
                                <a
                                    href="{{ url('/' . config('pertuk.route_prefix', 'docs') . '/' . $crumb['slug']) }}"
